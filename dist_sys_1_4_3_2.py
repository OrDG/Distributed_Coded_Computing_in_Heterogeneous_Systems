...
def get_receive_times_for_gradients_by_notifications(receive_requests_gradients, receive_requests_notifications):
    ...
    for num_gotten_task in range(num_workers):
        index_worker = MPI.Request.Waitany(receive_requests_notifications)
        MPI.Request.waitall(receive_requests_gradients[index_worker*num_params:(index_worker+1)*num_params])
        receive_times[index_worker] = time.time()
    if not MPI.Request.Testall(receive_requests_gradients):
        raise Exception('Didn\'t receive all grads')
...
def get_tp_kp_and_iter_times_and_update_finished_tasks_grads_and_vects(
    ...
    finished_time_until_task_vect = np.ones((num_tasks,), dtype='float') * started_iter_time
    for num_finished_task in range(num_tasks):
        index_finished_task = MPI.Request.Waitany(recv_task_vect_req)  # returns the finished req index
        ...
        finished_time_until_task_vect[index_finished_task] = time.time()
        ...
        update_tp_kp_time_if_needed(task_partition, task_split_vect, index_finished_task,
                                    finished_tp_kp_time)
    if not MPI.Request.Testall(recv_gradient_req):
        raise Exception('Didn\'t receive all grads')
    return finished_iter_time, finished_tp_kp_time, finished_time_until_task_vect
...
def append_part1_results_to_metadata_dict(start_time, end_tp_time, time_buffer, dict_part1):
    if np.all(end_tp_time > 0):
        ...
        tp_send_time_vect = tp_and_cp_time_vect - cp_time_vect - tp_calc_time_vect

        dict_part1['tp_time_list'].append(tp_calc_time_vect + tp_send_time_vect)
        dict_part1['tp_calc_time_list'].append(tp_calc_time_vect)
        dict_part1['tp_send_time_list'].append(tp_send_time_vect)
        dict_part1['cp_time_list'].append(cp_time_vect)


def get_timely_params_from_part_1(metadata_dict):
    tp_time = np.array(metadata_dict['tp_time_list'])
    mean_tp = np.mean(tp_time, axis=0)
    var_tp = np.var(tp_time, axis=0)
    mean_cp = np.mean(np.array(metadata_dict['cp_time_list']), axis=0)
    mean_tp_grad = np.mean(np.array(metadata_dict['tp_send_time_list']), axis=0)
    mean_tp_calc = np.mean(np.array(metadata_dict['tp_calc_time_list']), axis=0)
...
if __name__ == '__main__':
    ...
    model = torchvision.models.resnet50(weights=None).to(device).float()
    ...
    if rank == 0:  # ######################################### Master ################################################
        ...
        for num_tasks in range(k, m+1):
            ...
            # measures dict
            metadata_dict_part1 = {'cp_time_list': [], 'tp_time_list': [], 'tp_calc_time_list': [],
                                   'tp_send_time_list': []}

            print_to_console('Master: num_tasks = {}, k = {}, omega = {}, c = {}\n'.format(num_tasks,
                                                                                                k, omega, c))
            ...
            for num_split_run in range(2):
                ...
                metadata_dict_part2 = {'tp_kp_list': [], 'titer_list': [], 'time_until_task_list': []}
                ...
                for epoch in range(num_epocs_tp_kp):
                    for i, (images, labels) in enumerate(train_loader_CIFAR10):
                        ...
                        finished_iter_time, finished_tp_kp_time, finished_time_from_start = \
                        ...
                        metadata_dict_part2['tp_kp_list'].append(finished_tp_kp_time-start_iter_time)
                        metadata_dict_part2['titer_list'].append(finished_iter_time - start_iter_time)
                        metadata_dict_part2['time_until_task_list'].append(finished_time_from_start - start_iter_time)
                        # print_to_console('time since start iter = {}'.format(finished_time_from_start - start_iter_time))

                mean_tp_kp_vect, var_tp_kp_vect, mismatch, mean_time_iter = get_timely_params_from_part2(
                    metadata_dict_part2['tp_kp_list'], metadata_dict_part2['titer_list'])
                ...
                metadata_list.append({'m': m, 'd': d, 'num_tasks': num_tasks, 'K': k, 'omega':omega, 'C':c,
                                      'mean_tp_vect': mean_tp_vect, 'var_tp_vect': var_tp_vect,
                                      'mean_cp_vect': mean_cp_vect, 'metadata_dict_1': metadata_dict_part1,
                                      'mean_tp_calc_vect': mean_tp_calc_vect, 'mean_tp_send_vect':mean_tp_grad_vect,
                                      'task_split_vect': task_split_vect, 'metadata_dict_2': metadata_dict_part2,
                                      'mean_tp_kp_vect': mean_tp_kp_vect, 'var_tp_kp_vect': var_tp_kp_vect,
                                      'mismatch': mismatch, 'mean_titer': mean_time_iter})
        ...
        with open('data.pickle', 'wb') as handle:
            pickle.dump(metadata_list, handle, protocol=pickle.HIGHEST_PROTOCOL)
